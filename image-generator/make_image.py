...
def build_output_name(args, ext):
    """
    Build a standardised output filename based on dimensions + options.
    ext: file extension including dot, e.g. '.fits' or '.hdf5'
    """
    parts = []

    # Dimensions always first: e.g. 256x256x4
    dims_str = "x".join(str(d) for d in args.dimensions)
    parts.append(dims_str)

    # Options (only include ones actually used)

    if args.nans:
        parts.append("nan-" + "-".join(args.nans))

    if args.nan_density != 25.0:
        parts.append(f"nanpct-{args.nan_density}")

    if args.infs:
        parts.append("inf-" + "-".join(args.infs))

    if args.inf_density != 1.0:
        parts.append(f"infpct-{args.inf_density}")

    if args.checkerboard:
        parts.append(f"checker-{args.checkerboard}")

    if args.gaussian_model:
        parts.append(f"gauss-{int(args.gaussian_model[0])}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Optional: also write an HDF5 version of the output FITS file. "
            "If provided without a filename, an HDF5 filename will be auto-generated."
        ),
    )
    ...
    if not args.output:
        args.output = build_output_name(args, ".fits")

    if args.hdf5 is True:
        args.hdf5 = build_output_name(args, ".hdf5")
    ...
    if args.hdf5:
        ...
        fits_to_hdf5(args.output, str(h5_path))
